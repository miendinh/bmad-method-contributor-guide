...
> Detailed comparison of BMad against other AI agent frameworks (code-level) + migration paths from vanilla Claude Code to BMad, and from v5 to v6.
...
| | LangGraph | BMad |
|---|-----------|------|
| **Layer** | Runtime engine for agents | Prompt framework guiding agents |
...
**BMad:** You're building products with a PM/Architect/Dev workflow, team not ML-focused, want git-native artifacts.
...
Both: Multi-agent frameworks with roles.
...
| | CrewAI | BMad |
|---|--------|------|
| **Agent definition** | Python class with role, goal, backstory | YAML + TOML with name, title, role, identity, principles |
...
### BMad runs alongside Claude Code

BMad **doesn't replace** Claude Code — it **layers on top**:
...
Surprising comparison: BMad and Bazel/Make are both **declarative orchestration tools**.
...
If you like Bazel/Make's declarative style, BMad's philosophy should feel familiar.
...
**Use the Quick Dev path** (without going through full PRD/Architecture):
...
**Purpose:** Experience the BMad workflow without overhead.
...
**Commit everything to git.** You now have a full audit trail.
...
Let the v6 installer handle it.
...
BMad v6 features are evolving. Patterns to future-proof your customizations.

### Upcoming features (from the roadmap)
...
If you want a different behavior, create a new skill (`my-org-skill`) and invoke it from the agent menu.
...
When the framework updates, your facts are preserved.
...
### Migration strategy for upcoming BMad Builder
...
**Closest analog:** Bazel/Make but for AI reasoning instead of compilation.
...
**Read next:** [13-operational-runbook.md](13-operational-runbook.md) — Release process, troubleshooting, security.
