...
> Detailed specification of each core skill, sufficient to understand or reimplement. Each skill has: schema, workflow logic, state machine, edge cases, data shapes.
...
- **Files:** `SKILL.md` (143 lines), `methods.csv`
- **Type:** Inline skill (logic in SKILL.md, no workflow.md)
...
### Deep purpose
The skill is designed to **break LLM confirmation bias** — LLMs tend to validate their own output. By applying a **specific reasoning method**, it forces the LLM to look at output through a **specific angle** (vs a vague "try again").

### Input schema
- `content` (required) — Content just generated, in need of improvement
- Party-mode participants (optional) — Agents joining via `resolve_config.py`
- `methods.csv` — 28+ reasoning methods with category, method_name, description, output_pattern

### Output schema
- Enhanced version of the content
- Elicitation history (tracks methods already applied)
...
**Output pattern:** flexible guide, not a strict format
...
- **Files:** SKILL.md (7 lines), workflow.md (55 lines), template.md, brain-methods.csv, 8 step files
- **Type:** Micro-file workflow
...
### Deep purpose
**Anti-bias protocol:** LLMs tend toward semantic clustering (ideas converge). This skill forces the AI to **shift domain every 10 ideas** to maintain divergence.

**Quantity goal:** 100+ ideas before organizing — the first 20 are obvious, magic happens at 50-100.
...
- Initialize session file with frontmatter
...
**Rule:** After every 10 ideas in a technique, force a domain shift:
- Technical ⇌ UX
- Business ⇌ Edge cases
- Mainstream ⇌ Edge cases
- Orthogonal category

**Reason:** LLM sequential bias makes later ideas cluster semantically. Forced pivots maintain divergence.
...
- **Type:** Interactive skill with Python helper
...
### Deep purpose
Guide the user to write **customize override TOML** without needing to know TOML syntax. The skill translates plain English → TOML fragments and handles merge semantics correctly.
...
### Output schema
- Override file: `_bmad/custom/{skill-name}.toml` (team) or `.user.toml` (user)
...
- **Files:** SKILL.md (176 lines), `scripts/analyze_sources.py`, `agents/distillate-compressor.md`, `agents/round-trip-reconstructor.md`, `resources/compression-rules.md`, `resources/distillate-format-reference.md`, `resources/splitting-strategy.md`
- **Type:** Multi-agent spawning skill
...
- **Summary:** drops details, keeps essence
- **Distillate:** keeps every fact, decision, constraint, relationship. Strips formatting overhead.

**Use case:** Context for a downstream LLM workflow, e.g., PRD creation receiving a distilled product brief + research.
...
- Spawn final **merge compressor** with intermediate distillates as input
- Final compressor: cross-group dedup, thematic regroup, final compression

**Graceful degradation:** If the subagent is unavailable, the main agent reads sources and compresses inline.
...
3. **Save output** (single file or split folder)
4. **Measure** — run `analyze_sources.py` on the distillate to get the actual token count
5. **Report JSON:**
```json
{
  "status": "complete",
...
- The reconstructor ONLY sees the distillate (not originals)
- Receive the reconstruction
- Semantic diff:
  - Core info present?
...
- **Threshold:** ~5000 tokens for a single file
...
- **Files:** Only SKILL.md
- **Type:** Inline single-pass review

### Deep purpose
**CONTENT IS SACROSANCT.** The skill reviews TEXT (prose), not ideas.
- ❌ Does not challenge ideas: "This argument is weak"
- ✅ Challenges expression: "This sentence is ambiguous"

Microsoft Writing Style Guide is the baseline (unless `style_guide` overrides).

### Input schema
- `content` (required, ≥3 words)
- `style_guide` (optional) — override all principles
- `reader_type` (optional, default `humans`) — or `llm`
...
- Don't "fix" style the user chose deliberately
...
- Consistent terminology (don't use synonyms for the same concept)
- Explicit structure (prefer lists over prose for enumerable items)
- Avoid cultural references the LLM might miss
- Reference standards explicitly (not "as convention dictates")

**Step 4: Output**
- Deduplicate: the same issue in multiple places → one entry with locations listed
- Minimal fix: the smallest edit that resolves the issue
...
- **Files:** Only SKILL.md
- **Type:** Inline multi-step analysis

### Deep purpose
Structural editor. Propose **cuts, reorgs, simplifications** while preserving comprehension.
...
- **Files:** Only SKILL.md
- **Type:** Catalog-driven routing skill

### Deep purpose
Smart orientation: the user doesn't need to know the catalog of skills, only needs to ask "what next". The skill analyzes state + query → recommends next step.
...
**Phases:** `anytime` or numbered (`1-analysis`, `2-planning`, `3-solutioning`, `4-implementation`).
**Dependencies:** `after`, `before` — format `skill-name` or `skill-name:action`.
**Required:** gates — `required=true` blocks later phases.
**`_meta` rows:** module documentation URLs.

**2. Config:**
- `config.yaml` + `user-config.yaml` in `_bmad/` and subfolders
...
- Recommend running in a **fresh context window**
- Match the user's tone
- Never dump the entire catalog — surface relevant only
...
### Deep purpose
Generate navigation (`index.md`) for a folder containing multiple markdown files.
...
- **Files:** Only SKILL.md (128 lines)
- **Type:** Orchestration skill spawning real subagents
...
### Deep purpose
**Real independent thinking.** If one LLM roleplays multiple characters, "opinions" converge (performative). Spawning each agent as a **separate subagent process** → genuine diversity.
...
| User asks agent to respond to another | Just that agent with other's response as context |
| Rotate over time | Avoid same 2 dominating |

**2. Build context + spawn**

For each selected agent, spawn a subagent via the Agent tool with prompt:
...
**Spawn in parallel** — all Agent tool calls in a single response for concurrency.

**Solo mode:** Generate all responses yourself in a single message, each agent's icon + name header.
...
| "Winston, thoughts on Sally's?" | Spawn just Winston with Sally's response as context |
| "Bring in Amelia" | Spawn Amelia with summary |
| "I agree with John, go deeper" | Spawn John + 1-2 others |
| "What would Mary and Amelia think about Winston's?" | Spawn Mary + Amelia with Winston's response as context |
...
- **Files:** Only SKILL.md
- **Type:** Inline adversarial review
...
- **Files:** Only SKILL.md
- **Type:** Inline path-analysis review
...
**Orthogonal** to `bmad-review-adversarial-general` — both can run independently.
...
- No filler
- Strict JSON
- Scope: if a diff is provided, only diff hunks + directly reachable boundaries
...
### Deep purpose
**Problem:** A 50+ page PRD exceeds the LLM context window for a specific workflow (e.g., one epic).
...
Options presented:
- **Delete** (recommended) — Keeping source vs shards defeats the purpose
- **Move to archive** — Preserve but not in main location
- **Keep** (not recommended) — Duplicates info, confusing

### Rules

**Critical:** Keeping both original + sharded DEFEATS the purpose. User should choose Delete (default recommendation).
...
| # | Skill | Type | Primary use | Interaction type |
|---|-------|------|-------------|------------------|
...
- **Output in communication_language:** Speak the user's preferred language
- **Document output in document_output_language:** Artifacts in the configured doc language
- **Invoke syntax:** "Invoke the `skill-name` skill" (REF-03)
- **File references:** Relative within the skill, config variables across skills

---

**Continue reading:** [09b-skills-phase1-2-deep.md](09b-skills-phase1-2-deep.md) — Phase 1 + 2 skills in detail.
