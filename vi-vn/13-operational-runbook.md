...
**Response template cho can't-repro:**
```
...
- User's `_bmad/` configuration (includes secrets potentially)
...
**Glob expansion:**
- ✅ Resolved against fixed base (`{project-root}`)
...
**BMad doesn't execute code itself** — LLM reads markdown + generates code.
...
**Known limitation:** No sandboxing. If LLM writes `rm -rf /`, user must catch before execution.
...
- BMad warns if module has many transitive deps
...
1. **File copy:** 300+ files per module, sequential
   - **Potential fix:** Parallel copy với semaphore limit
   - **Impact:** ~30% faster on SSD

2. **SHA-256 hashing:** Every file on scan + update
   - **Potential fix:** Cache hashes với mtime check
...
**Cause:** BMad also installed in parent directory.
...
# If exist: manually restore from backup
```
...
**Cause:** User modified file that framework also updated.
...
BMad không ghi log file by default. If debug mode:
...
**Đọc tiếp:** [14-rewrite-blueprint.md](14-rewrite-blueprint.md) — Blueprint để rewrite BMad from scratch.
