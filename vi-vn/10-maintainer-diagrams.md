...
> Diagrams cho maintainers đào sâu kiến trúc installer, validator, build pipeline. Dùng khi debug hoặc rewrite.
...
2. [File-level data flow trong installer](#2-file-level-data-flow-trong-installer)
...
8. [Backup-restore flow khi update](#8-backup-restore-flow-khi-update)
...
Chi tiết 8-phase install với error paths.
...
## 2. File-level data flow trong installer

Dữ liệu flow giữa source, temp backups, và target dirs với checksum diff logic.
...
Shape-based merge với keyed detection.
...
`validate-skills.js` execution với 14 deterministic rules.
...
| **Sub-agent spawn** | Agent tool với prompt | Party mode, distillator |
| **External CLI** | `npx @kayvan/markdown-tree-parser`, `python3 resolve_*.py` | Deterministic logic |

---

## 8. Backup-restore flow khi update
...
### Decision table cho restoration
...
| Arrays of tables với `code` field | Merge by code (replace or append) |
| Arrays of tables với `id` field | Merge by id |
...
Chain dependencies giữa npm scripts.
...
10 maintainer-focused diagrams đủ để:
...
**Tổng cộng project có:**
- 16 user-facing diagrams (file 05)
- 10 maintainer diagrams (file này)
- Type variety: state, sequence, class, flowchart, graph

---

**Đọc tiếp:** [11-testing-and-quality.md](11-testing-and-quality.md) — Testing infrastructure + quality metrics.
