# 14. Rewrite Blueprint - Tái hiện BMad từ đầu
...
> **Mục đích:** Blueprint để rewrite BMad framework sang **Go / Rust / Python** hoặc bất kỳ ngôn ngữ nào. Interface specs + testing strategy + minimum viable framework.

> **Đối tượng:** Engineer muốn build BMad-compatible alternative, hoặc team muốn BMad internally với ngôn ngữ khác.
...
BMad framework gồm **3 phần chính**:
...
These are **declarative content**, không phải code. AI LLM đọc chúng directly.
...
- [ ] CLI tool để resolve: `resolve_customization --skill X --key agent`
...
- Update detection với file hash diff
...
See file [08-installer-code-level-spec.md § 5.3](08-installer-code-level-spec.md) cho full implementations. Summary:
...
// Concurrent file copy với semaphore
...
Gradual migration từ JS installer sang new language.
...
- Validate 100% identical findings với JS version
- Publish as standalone binary

Benefit: User gets fast validator, BMad team verifies port quality.

### Phase 2 (Month 3-4): Build resolver-only

- Port `resolve_customization.py` → new language
- Validate identical merged output
- Publish as standalone binary

### Phase 3 (Month 5-6): Build installer core

- Port `installer.js` state machine
- Dual-install mode: user can choose JS or new installer
...
Không phải full-featured, nhưng demonstrates core concepts.
...
- [ ] Installs compatible `_bmad/` structure
...
1. Read [08-installer-code-level-spec.md](08-installer-code-level-spec.md) for actual JS code
2. Pick target language, follow recipe above
3. Start với **validator only** (self-contained, easy to verify)
4. Build incrementally, test against JS reference

---

**Đọc hết file 14.** Full developer deep dive đã complete.

Quay lại [README.md](README.md) để xem index.
