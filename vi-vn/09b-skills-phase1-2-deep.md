...
- Enter flow from "BP" (brainstorm) → "CB" (product brief) → "WB" (PRFAQ stress test)
...
**mermaid-gen.md:** Interactive diagram creation với syntax validation
...
### Mục đích sâu
**Brownfield AI onboarding:** Khi apply BMad vào existing codebase, AI cần context. Skill này scan codebase + generate structured docs.

### 3 modes

1. **`initial_scan`** — First time, no docs exist
2. **`full_rescan`** — Refresh entire documentation
3. **`deep_dive`** — Focus on specific area (e.g., auth, billing)
...
- **Files:** SKILL.md (136 dòng), customize.toml, `assets/prfaq-template.md`, 4 stage references (press-release, customer-faq, internal-faq, verdict), 2 sub-agents (artifact-analyzer, web-researcher)
...
### Mục đích sâu
**Amazon's Working Backwards methodology.** Force viết press release cho sản phẩm hoàn thành **TRƯỚC khi xây**. Nếu không thể viết compelling press release, sản phẩm chưa sẵn sàng.

**Tone:** Tough love coaching. Challenge vague thinking. Reject weak ideas.

### Arguments
- `--headless` / `-H` — Autonomous first-draft từ provided context

### Input schema

**Required (headless mode):**
- `customer` — specific persona, không "everyone"
...
**Goal:** Raw concept on table, establish customer-first thinking.
...
**Create output** at `{planning_artifacts}/prfaq-{project_name}.md` using template.
...
**HALT:** When have enough to draft press release headline → route to `./references/press-release.md`.

#### Stage 2: Press Release (press-release.md)

**Goal:** Press release customer would stop scrolling for.
...
**HALT:** When press release coherent + compelling → route to `./customer-faq.md`.

#### Stage 3: Customer FAQ (customer-faq.md)

**Goal:** Validate value prop through hardest customer questions.

**Role:** Become the customer. Busy, skeptical, been burned. Read press release. Now questioning.
...
**DON'T generate softball:** "How do I sign up?" is CTA, not FAQ.
...
**HALT:** When every question has honest specific answer → route to `./internal-faq.md`.

#### Stage 4: Internal FAQ (internal-faq.md)

**Goal:** Stress-test from builder's side. "Can we pull this off — and should we?"
...
- **Files:** SKILL.md (118 dòng), customize.toml (48 dòng, defines `brief_template`), 4 stage prompts, 4 sub-agents, resources/brief-template.md
...
### Mục đích sâu
Create 1-2 page executive summary product brief với intelligent artifact analysis + web research.

**Design rationale:** Understand intent FIRST before scanning artifacts — without knowing topic, scanning is noise not signal.
...
**Goal:** Know WHY user is here + WHAT brief is about BEFORE anything else.

**Brief type detection:**
- Product → focus market differentiation
- Internal tool → focus adoption path
- Research project → focus stakeholder value
- Other → case-by-case
...
**Fast-track:** All 4 essentials in opening → confirm + move to Stage 2.

**Graceful redirect:** Can't articulate after 2-3 exchanges → suggest `bmad-brainstorming`.

**HALT:** When have enough → route to `prompts/contextual-discovery.md` with current mode.
...
**Transition:** "I think I have solid picture. Ready to draft, or anything else?"
...
- Ensure formatting clean

**Step 2: Offer distillate**

"Captured additional detail — [mention 2-3 examples]. Want detail pack for PRD creation?"
...
Dense bullets by theme, self-contained, includes:
...
Menu đơn giản (chỉ 1 item) — Sally's chủ yếu làm UX via CU.
...
- **Files:** SKILL.md (104 dòng), customize.toml, `steps-c/` (15 step files), `templates/prd-template.md`, `data/prd-purpose.md`
...
- How will we know product is working?
- Menu: [A/P/C]

**step-09-functional.md:**
- Functional requirements as **capability contract**
- Format: FR1, FR2, ... with:
  - Name
...
Each step has [A/P/C] menu → HALT → user 'C' proceeds.
...
- **Files:** SKILL.md (76 dòng), customize.toml, `steps/` (15 files), `ux-design-template.md`
...
- **Type:** Step-file workflow (4-5 steps, E-prefixed cho edit mode)
...
All 4 agents (Mary, Paige, John, Sally) follow same 8-step activation.

### Step-file architecture (3 workflows use)
...
### Distillate pattern
Both `bmad-prfaq` và `bmad-product-brief` produce:
- Main artifact (PRFAQ or Brief)
- Optional distillate (dense bullets for downstream)

Distillate feeds Phase 2 PRD creation với rich context mà main artifact elided.

---

**Đọc tiếp:** [09c-skills-phase3-deep.md](09c-skills-phase3-deep.md) — Phase 3 Solutioning skills chi tiết.
