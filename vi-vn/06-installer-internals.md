...
> Đặc tả cho **framework maintainer**: code internals của installer CLI, skill validator, reference validator, docs build pipeline. Đủ để modify/rewrite.
...
17. [Extension points (cho maintainer)](#17-extension-points-cho-maintainer)
...
**Phân tách:**
- `tools/installer/` — logic chạy khi user `npx bmad-method install`
- `tools/*.{js,mjs}` — maintainer tools (validate, build, format) chạy trong CI
- `tools/skill-validator.md` — spec của rules (đọc bởi validator + LLM)
...
### 2.2 Tại sao @clack/prompts thay vì Inquirer.js?

- Inquirer.js bug trên Windows với arrow keys (libuv #852)
- @clack/prompts không phụ thuộc libuv event loop processing
- API cleaner (intro/outro/select/multiselect/confirm)

### 2.3 Command pattern

Mỗi command là 1 file trong `commands/`:
...
Thêm command mới = thêm file vào `commands/`, không cần sửa `bmad-cli.js`.
...
```js
action: async (options) => {
  // Step 1: Gather config (interactive hoặc non-interactive với --yes)
  const config = await ui.promptInstall(options);
...
- Config passes through nhiều layers (UI → installer → module setup → IDE setup)
- Immutable prevents mutations giữa chừng
- Single source of truth cho install params
- Easier debug (snapshot at start vs end)
...
**Vì sao không dùng fs-extra?**

> fs-extra dùng graceful-fs monkey-patching, gây non-deterministic file loss trên macOS (issue #1779).

Solution: drop-in replacement dùng native `node:fs/promises`:
...
Có 3 loại module:
1. **Built-in** — `core`, `bmm` (shipped với package)
2. **Official** — từ `bmad-plugins-marketplace` registry
3. **Custom** — user `--custom-source <git-url>`
...
Dùng bởi `bmad-help` skill để route.
...
Deterministic validator — check 14 rules có thể verify bằng regex/AST, không cần LLM.
...
| SKILL-06 | SKILL.md | `description` length, có "Use when/if" |
| SKILL-07 | SKILL.md | Body non-empty sau frontmatter |
| WF-01 | Non-SKILL.md files | Frontmatter KHÔNG có `name:` |
| WF-02 | Non-SKILL.md files | Frontmatter KHÔNG có `description:` |
| PATH-02 | All files | KHÔNG dùng biến `installed_path` |
| STEP-01 | Step files | Filename format `step-NN[-variant]-desc.md` |
| STEP-06 | Step files | Frontmatter KHÔNG có name/description |
| STEP-07 | steps/ folder | Có 2-10 steps |
| SEQ-02 | All files | KHÔNG estimate thời gian (như "~5 minutes") |

### 11.3 Frontmatter parser (hand-rolled)

Validator không dùng `yaml` library — hand-rolled parser để avoid dependency:
...
Validate tất cả file references trong YAML/MD/XML/CSV files resolve được.
...
Exit 1 if any broken reference. Dùng trong CI.
...
Format workflow files với consistent style:
...
Auto-fix broken doc links:
- Scan all `.md` files
- Find broken `[text](path)` references
- Try to find intended target (fuzzy match)
- Offer fix

### 14.3 migrate-custom-module-paths.js

Migration script cho v4→v5 upgrades — old custom module paths → new structure.

### 14.4 validate-svg-changes.sh

Bash script validate SVG changes (for icon updates).
...
```js
// Pure Node test, không dùng framework
...
Delay expensive imports, ESM interop with CommonJS.
...
Single summary output thay vì scattered logs.
...
Generic `ConfigDrivenIdeSetup` reads config, applies logic. Thêm IDE mới = add YAML entry, không code thêm.
...
## 17. Extension points (cho maintainer)
...
1. Edit `tools/platform-codes.yaml`:
   ...
   ```
2. Test `npx bmad-method install --tools my-ide`
3. `ConfigDrivenIdeSetup` tự handle phần còn lại

### 17.2 Thêm validation rule

**Option A: Deterministic rule trong validate-skills.js**
...
**Option B: Inference rule trong skill-validator.md**
...
- **Detection:** [How LLM detects]
- **Fix:** [How to fix]
```

LLM sẽ apply rule qua inference.

### 17.3 Thêm command CLI
...
1. Create `tools/installer/commands/my-command.js`:
   ...
   ```
2. Auto-loaded bởi `bmad-cli.js`
3. Test: `npx bmad-method my-command --xxx value`

### 17.4 Thêm module registry source

Currently: GitHub `bmad-plugins-marketplace`.

Để thêm source khác:
1. Edit `tools/installer/modules/external-manager.js`
2. Thêm method `fetchFromXXX()`
3. Update `loadExternalModulesConfig` để try sources in priority order

### 17.5 Override manifest format

Manifest CSV/YAML hiện tại có fixed schema. Để thay đổi:
1. Edit `tools/installer/core/manifest-generator.js`
2. Update `writeSkillManifest()`, `writeFilesManifest()`
3. Migration script cho existing manifests

---

## Tóm lược

Installer là **framework modular** với:
- Config-driven IDE setup (add YAML entry, not code)
...
- State machine rõ ràng (detect → prepare → install → restore)

Validator là **deterministic + inference hybrid**:
- 14 deterministic rules (regex/AST, no LLM)
- 13+ inference rules (LLM reads `skill-validator.md`)
- Self-documenting (rule markdown = human spec + machine check + LLM prompt)

Docs build dùng **Astro + Starlight** với LLM-friendly artifacts (`llms.txt`, `llms-full.txt`).

Extension points: platform-codes.yaml (IDE), skill-validator.md (rules), commands/*.js (CLI), modules/external-manager.js (registry).

---

**Đọc tiếp:** [07-extension-patterns.md](07-extension-patterns.md) — Patterns để mở rộng framework.
