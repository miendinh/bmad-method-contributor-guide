...
> Đi sâu step-by-step 2 workflows quan trọng nhất: `bmad-create-prd` (15 steps, step-file architecture) và `bmad-retrospective` (12 steps, XML inline party-mode). Deep dive cho developer muốn hiểu exact mechanics.

> **Note:** File này bổ sung cho [09b](09b-skills-phase1-2-deep.md) và [09d](09d-skills-phase4-deep.md) — focus ở đây là MECHANICS từng step.
...
**Architecture:** Step-file (micro-file), 15 files, ~2900 dòng tổng.
...
## Step 01: Init (178 dòng)

**Goal:** Khởi tạo workflow PRD, detect continuation, discover input docs, setup document structure.

**Actions:**
1. Check if `{outputFile}` (prd.md) exists
2. If exists:
   - Read frontmatter
   - If `step-12-complete` in `stepsCompleted` → ask overwrite/cancel
   - Else → auto-route to `step-01b-continue` (resume)
3. If fresh:
   ...
   - Present found docs to user, ask which to load
   - Load confirmed docs
...
**HALT:** Wait for user 'C' before proceeding.

**Transition:** → step-02-discovery (normal) OR step-01b-continue (existing workflow detected)

---

## Step 01b: Continue (161 dòng)

**Goal:** Resume workflow from last incomplete step.

**Actions:**
1. Analyze current state from frontmatter:
   - `stepsCompleted` array → determine last completed step
   - `inputDocuments` → reload these files
2. Offer options to user:
   - `[R] Resume` from next step (after last completed)
   - `[C] Continue` with next logical step
   - `[O] Overview` — show current PRD state
   - `[X] Start over` — archive current, start fresh
3. If R/C: Load `step-NN-*.md` for next step
4. If O: Display summary, re-present menu
5. If X: Archive current prd.md → prd-archived-{date}.md, goto step-01

**Variables:**
- Uses: `{stepsCompleted}`, `{inputDocuments}`

**HALT:** User chooses R/C/O/X.

---

## Step 02: Discovery (208 dòng)
...
2. Determine scope:
   ...
   - Scale expectations?
3. Extract user context from loaded input docs
4. Reflect understanding back to user for validation
...
**Conditionals:**
- If vision missing → offer step-02b-vision (sidecar)
- If exec summary missing → offer step-02c-executive-summary (sidecar)
...
## Step 04: Journeys (201 dòng)

**Goal:** User journey mapping.

**Actions:**
1. Identify primary user personas (1-3 max)
2. For each persona, map journey:
...
## Step 05: Domain (194 dòng)

**Goal:** Domain-specific requirements.

**Actions:**
1. Based on classification from step-02:
...
**Skip** if domain-agnostic product.

**Menu:** `[A/P/C]`

---

## Step 06: Innovation (211 dòng)

**Goal:** Innovation + competitive differentiation.

**Actions:**
1. List 3-5 competitive alternatives (from research or web)
2. For each, identify:
   - Their value prop
   - Their weakness
3. Identify your differentiation:
   - Unique angle? (not just "better UX")
   - Defensible moat? (network effect, data, cost, expertise)
   - Why now? (timing rationale)
4. Stress-test: Is differentiation real or perceived?

**Menu:** `[A/P/C]`

---

## Step 07: Project Type (222 dòng)
...
## Step 08: Scoping (263 dòng, LONGEST)

**Goal:** MVP scoping, scope boundaries.

**Actions:**
1. List everything user wants (brainstorm)
2. Force ruthless cut:
   - **In scope (MVP):** Must-haves for launch
   - **Out of scope:** Explicitly cut
   - **Future considerations:** Phase 2, 3
3. Apply MVP principles:
   - Minimum viable — smallest thing that creates value
   - One primary use case
   - Cut features that don't hit primary metric (step-03)
4. Document rationale for each cut
5. Warn if scope seems large:
...
- "If budget halved, what survives?"

**Menu:** `[A/P/C]`

---

## Step 09: Functional Reqs (219 dòng)

**Goal:** Functional requirements as capability contract.
...
## Step 10: Non-Functional Reqs (230 dòng)
...
2. Format each NFR with measurable target
...
## Step 11: Polish (221 dòng)

**Goal:** Polish for flow + coherence.

**Actions:**
1. Review full document linearly
2. Check:
   - Consistent terminology?
   - FR numbers sequential?
   - Coverage complete (every journey → FR)?
...
3. Show final file path

**Output:** `{planning_artifacts}/prd.md` với frontmatter `status: 'complete'`
...
**Patterns observed:**
- Every main step has `[A/P/C]` menu
- HALT at every menu
- Append-only document building
- Frontmatter tracks `stepsCompleted`
- Sidecars (02b, 02c) optional
- Final step offers chaining to related skills
...
**Architecture:** XML inline workflow trong `workflow.md` (~1500 dòng).

**Output:** `{implementation_artifacts}/epic-{{epic_num}}-retro-{date}.md`

**Unique feature:** Party-mode — ALL dialogue format `"Name (Role): dialogue"`.
...
- **Architecture** (`FULL_LOAD`): Complete doc for context
- **PRD** (`FULL_LOAD`): Complete doc
...
**Goal:** Rich context setup before discussion.
...
**Dialogue example (from actual workflow):**
```
...
**Goal:** Concrete action items + detect if epic plan needs update.
...
**Actions:**
- Generate comprehensive retro document (see template in [09d §4-11](09d-skills-phase4-deep.md))
...
- **{user_name}** always Project Lead
- **Amelia** always facilitator
- **Never blend** responses — preserve voice
...
| When | Behavior |
|------|----------|
| After append-content step với `[A/P/C]` menu | HALT, wait for C |
...
- Last step often offers next skill:
...
1. Detects existing output file
2. Offers Resume option if incomplete
3. Reloads input documents từ frontmatter
4. Continues from last completed step

---

## Takeaway: Skill maturity levels

Observed 2 levels trong 2 workflows này:

**Level 1: Step-file architecture** (create-prd)
- Each step = separate file (2-5KB)
...
- Complex logic in single workflow.md
...
**Đọc tiếp:** [17-cheat-sheet.md](17-cheat-sheet.md) — 1-page cheat sheet.
