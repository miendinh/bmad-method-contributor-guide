...
> Testing infrastructure, test patterns, and quality metrics of the BMad framework. For developers who want to contribute tests and maintainers who want to extend test coverage.
...
5. [Integration tests for skills](#5-integration-tests-for-skills)
...
| `1` | Failures (test failed, lint errors, HIGH+ validation findings with `--strict`) |
...
BMad **does not use a test framework** (jest, mocha, vitest). Plain Node.js `assert`:
...
2. **Add fixtures with descriptive names:**
...
- **One concern per fixture:** Don't test multiple things in a single file
- **Self-documenting:** Name tells you what it tests

---

## 5. Integration tests for skills

BMad **has no automated integration tests** for full skill execution (requires LLM). Manual flow:
...
Disabled rules (specific to BMad):
- `MD013` (line-length) — Mermaid blocks are long
- `MD033` (inline-html) — XML workflow tags
- `MD041` (first-line-h1) — Some files have frontmatter only
...
There is no automated test framework for skill execution. Pattern:

**1. Unit-test the parser if there is one:**
```javascript
...
Testing philosophy:
- **Deterministic code:** Unit tests required
...
**Read next:** [12-comparison-and-migration.md](12-comparison-and-migration.md) — Detailed comparison with other frameworks + migration paths.
