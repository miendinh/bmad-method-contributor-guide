...
> BMad's key flows visualized with Mermaid: architecture, lifecycle, workflow, customization, validation, installer.
...
Top-level view: the layers that make up the BMad framework.
...
From idea to shipped code, the flow passes through 4 phases.
...
Relationships between Module, Agent, Skill, Workflow, Step.
...
When the user invokes `/bmad-agent-pm` — what actually happens.
...
How variables are resolved across multiple layers.
...
The 10 steps of dev-story with validation gates.
...
Micro-file architecture with 4 branching paths.
...
The state of a full sprint (from planning to retrospective).
...
The states a single story moves through.
...
How `npx bmad-method install` operates.
...
How `npm run quality` runs skill validation.
...
How party-mode spawns multiple subagents in parallel.
...
`bmad-correct-course` — when and how to pivot.
...
How artifact files are produced and consumed.
...
End-to-end: user has an idea and ships code, passing through each agent.
...
An alternative view: the hierarchy structure.
...
**Continue reading:** [06-installer-internals.md](06-installer-internals.md) — Installer, validator, and build internals.
