...
> **Phase 4 - Implementation:** The most complex BMM phase. Dev-story with RED-GREEN-REFACTOR + 8-level validation gate. Party-mode retrospective. Mid-sprint change management.
...
The broadest menu across all agents (7 items).
...
**Discovery strategy:** `SELECTIVE_LOAD` for epics + previous story, `FULL_LOAD` for architecture + PRD.
...
- **Files:** SKILL.md (7 lines), `workflow.md` (~450 lines of XML), `checklist.md`
...
- Load `{project_context}` for coding standards
...
**Parallel adversarial review** with 3 layers. Structured triage into actionable categories.
...
### Tip
Run with a **different LLM** than the implementer (catches bugs the original missed).
...
- **Missing spec:** Continue with bare-commit path
...
**Quick Flow path** (without formal PRD / Architecture / Story).

**Suitable for:**
- 1-15 stories
- Bug fixes
- Small features
- Established projects (already have architecture)
...
- **Files:** SKILL.md, `workflow.md` (XML, very long)
...
**Party-mode:** ALL dialogue uses the `"Name (Role): dialogue"` format with team members actively participating.
...
**Priority logic:**
1. **PRIORITY 1:** Check `sprint-status.yaml` → find highest epic with ≥1 story marked "done"
...
- Dialogue continues with genuine emotions (frustration, pride, concern)
...
**Read next:** [10-maintainer-diagrams.md](10-maintainer-diagrams.md) — 10 Mermaid diagrams for maintainers.
