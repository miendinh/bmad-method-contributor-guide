...
> **Purpose:** A detailed specification thorough enough to rewrite the BMad installer in Go/Rust/Python. Spec-level, code-ready, with actual algorithms + data shapes + function signatures.

> **Audience:** Framework maintainers who want to hack on the installer, or engineers porting BMad to another language.
...
9. [Interface specs for rewrite](#9-interface-specs-for-rewrite)
...
**Key insight:** Commands are not hardcoded — dropping a file into `commands/` is sufficient.

**Port considerations:**
- Go: use `cobra` for the CLI
- Rust: use `clap`
- Python: use `click` or `typer`

---

### 2.2 prompts.js (704 lines)

**Purpose:** A @clack/prompts wrapper with custom enhancements.

**Key patterns:**

**Lazy ESM loading** (to avoid expensive imports):
```javascript
let _clack = null;
...
**Custom autocompleteMultiselect:** Workaround for a @clack/prompts quirk — SPACE doesn't toggle by default.
...
**Port considerations:**
- Go: `charmbracelet/bubbletea` or `AlecAivazis/survey`
- Rust: `dialoguer` + `indicatif`
- Python: `questionary` or `rich.prompt`

---

### 2.3 fs-native.js (117 lines)

**Purpose:** Drop-in replacement for fs-extra using native `node:fs/promises`.

**Why not fs-extra?** fs-extra uses `graceful-fs` monkey-patching, which causes non-deterministic file loss on macOS (issue #1779).
...
**Port considerations:** In Go/Rust/Python the native file API is already sufficient — this wrapper is not needed.

---

### 2.4 project-root.js (132 lines)

**Purpose:** Locate the BMAD project root and module paths.
...
3. Create module directories (from `directories:` field in module.yaml)
...
**_prepareUpdateState():** Back up user files
```javascript
...
**detectCustomFiles():** File preservation scan
```javascript
...
Config is **immutable** — `Object.freeze()` is applied to `modules`, `ides`, and the whole object.
...
Preserves user modifications while updating. Uses SHA-256 + mtime comparison.
...
**Installed-skills scanning algorithm:**
...
The validator does not rely on a `yaml` library — a hand-rolled parser avoids the dependency:
...
**Fetched from:** `github.com/bmad-code-org/bmad-plugins-marketplace/registry/official.yaml`
...
4 variants are supported (searched in order):
...
- Manifest file parsers/writers (YAML, CSV)
- File ops with SHA-256
...
- Interactive prompts (via a TUI library)
- Command dispatch
- Status/uninstall/upgrade commands

**Phase 6 (Week 6): Validator**
- 14 deterministic rules
- Hand-rolled frontmatter parser (native to the language)
- JSON + strict output modes

**Phase 7 (Week 7): Build pipeline** (optional — only if the build tools are rewritten too)
...
The BMad installer has 3 layers:

1. **CLI/UI layer** (~2300 lines of JS): bmad-cli, prompts, ui
2. **Install logic layer** (~2000 lines of JS): installer, config, paths, manifest-generator
3. **Integration layer** (~1500 lines of JS): modules/*, ide/*, file-ops, fs-native

Core algorithms:
- **syncDirectory**: SHA-256 + mtime for file preservation
- **detectCustomFiles**: Scan + compare against manifest hashes
- **collectSkills**: Recursive walk, parse SKILL.md frontmatter
- **cloneExternalModule**: Git clone/fetch, with npm install if dependencies exist

The 14-rule validator: hand-rolled YAML parser, regex-based detection.

IDE setup: config-driven from `platform-codes.yaml`, with verbatim skill copy.

Estimated rewrite effort: **6-7 weeks** for a single maintainer experienced with the target language.

---

**Continue reading:** [09a-skills-core-deep.md](09a-skills-core-deep.md) — 12 core skills in detail.
